...
/**
 * An AggregateSchedule holds information regarding all possible schedules that can be completed with the desired task.
 ...
 * Task("6", 3, 5, 9);
 * <p>
 * Task 6 has enough time to also accomplish Task 1 or Task 2.  The AggregateSchedule holds both of those potential schedules in
 * the scheduledTasks array.
 * <p>
 * The AggregateSchedule's unScheduledTasks boolean array is used to determine whether a scheduled task has caused it to inherit
...
public class AggregateSchedule {
     ...
     */
    public AggregateSchedule(int size) {
        for (int i = 0; i < size; i++) {
            unScheduledTasks.add(i, true);
        }
    }

    public ArrayList<Schedule> getScheduledTasks() {
    ...
    public void addTask(Task task, int toDoListIndex) {
        Schedule schedule = new Schedule(task);
        unScheduledTasks.set(toDoListIndex, false);
        scheduledTasks.add(schedule);
    }

    public void addTasks(Schedule toDoListIndex, ArrayList<Boolean> add) {
